...
from datasets.simulation.dataset import Simulation
...
parser.add_argument('--batch_size', type=int, default=20, help='int type, batch size for training (default: 256)')
...
# Log
parser.add_argument('--log_directory', type=str, default='./logs/simulation', help='directory to generatethe json log file (default: ./logs)')
...
if __name__ == '__main__':
    ...
    dataset = partial(Simulation, dataset_name=args.dataset_name, data_dir=args.data_directory, cutoff_rate=args.cutoff_rate, 
                      virtual_channels=args.virtual_channel, delta_t=20, device=args.device)
    ...
    if args.model == 'FastEGNN':
        model = FastEGNN(node_feat_nf=2, node_attr_nf=0, edge_attr_nf=2, hidden_nf=args.dim_hidden, virtual_channels=args.virtual_channel, 
                            device=args.device, n_layers=args.num_layer, residual=True, attention=args.attention_required, 
                            normalize=args.direction_vector_normalize_required, tanh=args.tanh_required, gravity=[0, -1, 0])
        ...
        model = SchNet(hidden_channels=args.dim_hidden, max_num_neighbors=100000, cutoff=0.035, device=args.device)
    else:
