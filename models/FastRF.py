...
class E_GCL_vel(nn.Module):
    ...
    def coord_model_vel(self, node_feat, coord, node_vel, edge_index, coord_diff, edge_feat, virtual_edge_feat, virtual_coord_diff, node_vel_norm):
        row, col = edge_index
        ...
        # Change to RF
        coord = coord + node_vel * self.coord_mlp_vel(node_vel_norm)
    ...
    def forward(self, node_feat, edge_index, coord, node_vel, virtual_coord, virtual_node_feat, data_batch, edge_attr=None, node_attr=None):
        ...
        '''
        node_vel_norm = torch.norm(node_vel, p=2, dim=-1).unsqueeze(-1).detach()
        ...
        # Coord Model
        coord = self.coord_model_vel(node_feat, coord, node_vel, edge_index, coord_diff, edge_feat, virtual_edge_feat, virtual_coord_diff, node_vel_norm)
...
class FastRF(nn.Module):
    def __init__(self, node_feat_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels, device='cpu', act_fn=nn.SiLU(), 
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, E_GCL_vel(hidden_nf, hidden_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels=virtual_channels,
