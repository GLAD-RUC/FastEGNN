...
from models.se3_dynamics.models import OursTFN as TFN
...
class TFN_GCL_vel(nn.Module):
    ...
    def __init__(self, node_feat_nf, node_feat_out_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels, num_degrees=2, 
        ...
        super(TFN_GCL_vel, self).__init__()
        ...
        # Single Layer TFN
        self.tfn_layer = TFN(num_layers=1, num_channels=1, edge_dim=0, div=1, act_fn=act_fn, num_degrees=2)
    ...
    def coord_model_by_tfn(self, xs, vs, charges, edge_index):
        device = xs.device
    ...
    def coord_model_vel(self, node_feat, charges, coord, node_vel, edge_index, coord_diff, edge_feat, virtual_edge_feat, virtual_coord_diff):
    ...
    def forward(self, node_feat, charges, edge_index, coord, node_vel, virtual_coord, virtual_node_feat, data_batch, edge_attr=None, node_attr=None):
        '''
        ...
        # Update by TFN
        coord = self.coord_model_by_tfn(xs=coord, vs=node_vel, charges=charges, edge_index=edge_index).view(-1, 3)
        
        # Coord Model
        coord = self.coord_model_vel(node_feat, charges, coord, node_vel, edge_index, coord_diff, edge_feat, virtual_edge_feat, virtual_coord_diff)
...
class FastTFN(nn.Module):
    def __init__(self, node_feat_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels, device='cpu', act_fn=nn.SiLU(), 
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, TFN_GCL_vel(hidden_nf, hidden_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels=virtual_channels,
                                                    act_fn=act_fn, residual=residual, attention=attention, normalize=normalize, tanh=tanh, gravity=gravity))
        self.to(self.device)

    def forward(self, node_feat, charges, node_loc, node_vel, edge_index, data_batch, loc_mean, edge_attr=None, node_attr=None):
        ...
        for i in range(0, self.n_layers):
            node_feat, node_loc, virtual_node_feat, virtual_node_loc = \
                  self._modules["gcl_%d" % i](node_feat, charges, edge_index, node_loc, node_vel, virtual_node_loc,
                   virtual_node_feat, data_batch, edge_attr=edge_attr, node_attr=node_attr)
